...
- `updater.github_owner = "bellenne"`
- `updater.github_repo = "scanserver"`

??? ??????????? ??????????:
1. ?????????? `APP_VERSION` ? `core/app_meta.py`.
2. ????????? ????? ??????????.
3. ?????????? GitHub Release ? ????? ??????, ???????? `v0.1.1`.
4. ????????????? zip-????? ?? ???????.
5. ?????? zip ?????? ?????????? `ScanServer.exe` ? ????????? ????? ??????.

?????????:
- ???????? ?????????? ???? ??? ?????? ??????????.
- ???? ?????? ????? ????? ??????? ??????, ?????????? ????????? zip, ???????? ????? ? ???????????????.
- ????????? `config.json`, `state.json`, `users_cache.json` ? `.tts_cache` ???????????.
